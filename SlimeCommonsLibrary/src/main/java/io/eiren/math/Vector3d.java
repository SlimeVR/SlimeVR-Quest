...
public class Vector3d implements Cloneable {
	...
	public String toString() {
		return new StringBuilder("Vector3D{").append(x).append(',').append(y).append(',').append(z).append('}').toString();
	}
	...
	public double squaredDistance(double toX, double toY, double toZ) {
		return (this.x - toX) * (this.x - toX) + (this.y - toY) * (this.y - toY) + (this.z - toZ) * (this.z - toZ);
	}
